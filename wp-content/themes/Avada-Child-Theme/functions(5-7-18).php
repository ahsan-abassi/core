...
//    echo '</pre>';
    ...
    while( $query->have_posts() ):$query->the_post();
        $thumbnail_img_url = get_the_post_thumbnail_url(get_the_ID(),'thumbnail');
        $data .= '<div class="fusion-layout-column fusion_builder_column fusion_builder_column_1_3  fusion-one-third front-core our-people-main  id="'.get_the_ID().'">';
        $data .= '<ul>';
        $featured_img_url = get_the_post_thumbnail_url(get_the_ID(),'portfolio-full');
